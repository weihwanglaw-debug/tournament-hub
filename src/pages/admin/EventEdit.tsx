      ...
      <div className="mb-8 p-8" style={{ border: "1px solid var(--color-table-border)" }}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <FF label="Prospectus PDF">
            {editing
              ? <input type="file" accept=".pdf" className="field-input" onChange={e => { const file = e.target.files?.[0]; if (file) set("prospectusUrl", file.name); }} />
              : <p className="text-sm opacity-60">{form.prospectusUrl || "No prospectus uploaded"}</p>}
          </FF>
          <div className="md:col-span-2">
            <p className="block text-xs font-semibold mb-2 opacity-70">Consent Statement</p>
            <div className="p-3 text-xs opacity-60" style={{ border: "1px solid var(--color-table-border)", backgroundColor: "var(--color-row-hover)" }}>
              Managed globally in{" "}
              <a href="/admin/config" className="underline" style={{ color: "var(--color-primary)" }}>Master Configuration</a>
            </div>
          </div>
        </div>
      </div>
