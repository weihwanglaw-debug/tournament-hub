...
function EventGallery({ images }: { images: string[] }) {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0) setLightboxIdx((lightboxIdx + 1) % images.length);
      else setLightboxIdx((lightboxIdx - 1 + images.length) % images.length);
    }
    ...
    <div className="mb-12">
      ...
      <AnimatePresence>
        {lightboxIdx !== null && (
          ...
          >
            <button className="absolute top-4 right-4 p-2 text-white/70 hover:text-white z-10" onClick={() => setLightboxIdx(null)}>
              <X className="h-6 w-6" />
            </button>
            {images.length > 1 && (
              <>
                <button className="absolute left-4 p-2 text-white/70 hover:text-white z-10"
                  onClick={e => { e.stopPropagation(); setLightboxIdx((lightboxIdx - 1 + images.length) % images.length); }}>
                  <ChevronLeft className="h-8 w-8" />
                </button>
                <button className="absolute right-4 p-2 text-white/70 hover:text-white z-10"
            ...
            <div className="absolute bottom-4 text-white/60 text-sm">
              ...
              <span className="ml-3 text-xs opacity-50 md:hidden">← Swipe to navigate →</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
